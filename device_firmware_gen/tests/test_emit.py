...
def _read(out_dir, module, suffix):
    path = os.path.join(out_dir, f"{module}_{suffix}")
...
class TestTypesHeader:
    def test_pragma_once(self, simple_rm, out_dir):
        ...
        assert "#pragma once" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_word_typedef_uint32(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_types.hpp")
    ...
    def test_word_typedef_uint8(self, tmp_path, out_dir):
        ...
        rm.generate()
        _gen(rm, out_dir)
        content = _read(out_dir, "tiny_mod", "reg_types.hpp")
    ...
    def test_word_typedef_uint16(self, tmp_path, out_dir):
        ...
        rm.generate()
        _gen(rm, out_dir)
        content = _read(out_dir, "sm_mod", "reg_types.hpp")
    ...
    def test_reg_status_bad_address(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "BAD_ADDRESS" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_reg_status_register_overflow(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "REGISTER_OVERFLOW" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_reg_status_out_of_range(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "OUT_OF_RANGE" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_reg_access_read(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "READ" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_reg_access_write(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "WRITE" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_reg_access_read_write(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "READ_WRITE" in _read(out_dir, "test_mod", "reg_types.hpp")

    def test_enum_class_for_enum_reg(self, enum_rm, out_dir):
        _gen(enum_rm, out_dir)
        content = _read(out_dir, "enum_mod", "reg_types.hpp")
    ...
    def test_namespace_wraps_content(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_types.hpp")
        assert "namespace test_mod_regs" in content

    def test_generated_header_comment(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "do not edit" in _read(out_dir, "test_mod", "reg_types.hpp")
...
class TestStorageHeader:
    def test_pragma_once(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "#pragma once" in _read(out_dir, "test_mod", "reg_storage.hpp")

    def test_includes_types_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_types.hpp" in _read(out_dir, "test_mod", "reg_storage.hpp")

    def test_reg_map_t_struct(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "RegMap_t" in _read(out_dir, "test_mod", "reg_storage.hpp")

    def test_extern_regs_declaration(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "extern RegMap_t regs" in _read(out_dir, "test_mod", "reg_storage.hpp")

    def test_single_word_value_member(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "word_t value = 0" in _read(out_dir, "test_mod", "reg_storage.hpp")

    def test_bank_entries_array(self, banked_rm, out_dir):
        _gen(banked_rm, out_dir)
        assert "entries[4]" in _read(out_dir, "bank_mod", "reg_storage.hpp")

    def test_multiword_words_array(self, multiword_rm, out_dir):
        _gen(multiword_rm, out_dir)
        assert "words[" in _read(out_dir, "mw_mod", "reg_storage.hpp")

    def test_namespace_wraps_content(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "namespace test_mod_regs" in _read(out_dir, "test_mod", "reg_storage.hpp")
...
class TestStorageSource:
    def test_includes_storage_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_storage.hpp" in _read(out_dir, "test_mod", "reg_storage.cpp")

    def test_reg_map_definition(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "RegMap_t regs" in _read(out_dir, "test_mod", "reg_storage.cpp")

    def test_zero_initialised(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "= {}" in _read(out_dir, "test_mod", "reg_storage.cpp")
...
class TestDeviceHeader:
    ...
    def test_bitfield_setter(self, tmp_path):
        ...
        content = open(os.path.join(out, "wbf_mod_reg_device.hpp"), encoding="utf-8").read()
...
class TestCommHeader:
    def test_pragma_once(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "#pragma once" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_read_declaration(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_read(" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_write_declaration(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_write(" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_reset_declaration(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_reset" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_info_struct(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "RegInfo" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_addr_slot_struct(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "RegAddrSlot" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_includes_types_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_types.hpp" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_status_return_type(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_comm.hpp")
...
class TestCommSource:
    def test_includes_comm_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_comm.hpp" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_includes_storage_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_storage.hpp" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_address_table_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_comm.cpp")
        assert "reg_table" in content

    def test_reg_info_table_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "RegInfo" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_reg_reset_uses_memset(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "memset" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_reg_reset_function_body(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_reset" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_multiword_buffers_present(self, multiword_rm, out_dir):
        _gen(multiword_rm, out_dir)
        content = _read(out_dir, "mw_mod", "reg_comm.cpp")
    ...
    def test_multiword_read_uses_memcpy(self, multiword_rm, out_dir):
        _gen(multiword_rm, out_dir)
        assert "memcpy" in _read(out_dir, "mw_mod", "reg_comm.cpp")
    ...
    def test_critical_macros_in_types_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_types.hpp")
        assert "TEST_MOD_REG_ENTER_CRITICAL" in content
        assert "TEST_MOD_REG_EXIT_CRITICAL" in content

    def test_critical_macros_used_in_comm_source(self, multiword_rm, out_dir):
        _gen(multiword_rm, out_dir)
        content = _read(out_dir, "mw_mod", "reg_comm.cpp")
        assert "MW_MOD_REG_ENTER_CRITICAL" in content
        assert "MW_MOD_REG_EXIT_CRITICAL" in content

    def test_namespace_wraps_content(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "namespace test_mod_regs" in _read(out_dir, "test_mod", "reg_comm.cpp")
...
class TestVerifySource:
    def test_do_not_edit_comment(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "do not edit" in _read(out_dir, "test_mod", "reg_verify.cpp")

    def test_includes_device_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_device.hpp" in _read(out_dir, "test_mod", "reg_verify.cpp")

    def test_includes_comm_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_comm.hpp" in _read(out_dir, "test_mod", "reg_verify.cpp")

    def test_verify_function_declaration(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "int test_mod_reg_verify()" in _read(out_dir, "test_mod", "reg_verify.cpp")

    def test_verify_assert_macro_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_verify.cpp")
        assert "REG_VERIFY_ASSERT" in content

    def test_returns_zero_on_success(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "return 0;" in _read(out_dir, "test_mod", "reg_verify.cpp")
    ...
    def test_bitfield_accessors_in_verify(self, bitfield_rm, out_dir):
        _gen(bitfield_rm, out_dir)
        content = _read(out_dir, "bf_mod", "reg_verify.cpp")
...
class TestHostShim:
    def test_do_not_edit_comment(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "do not edit" in _read(out_dir, "test_mod", "reg_host.cpp")

    def test_host_test_guard_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        content = _read(out_dir, "test_mod", "reg_host.cpp")
        assert "#ifndef AURA_HOST_TEST" in content
        assert "error" in content  # #  error "..." directive

    def test_includes_comm_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_comm.hpp" in _read(out_dir, "test_mod", "reg_host.cpp")

    def test_includes_storage_header(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_storage.hpp" in _read(out_dir, "test_mod", "reg_host.cpp")
    ...
    def test_main_function_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "int main()" in _read(out_dir, "test_mod", "reg_host.cpp")
    ...
    def test_vector_used_for_io_buffers(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "std::vector" in _read(out_dir, "test_mod", "reg_host.cpp")

    def test_forward_declaration_verify(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "test_mod_reg_verify" in _read(out_dir, "test_mod", "reg_host.cpp")
    ...
    def test_atomic_statuses_send_data(self, simple_rm, out_dir):
        ...
        _gen(simple_rm, out_dir)
        assert "s <= 2" in _read(out_dir, "test_mod", "reg_host.cpp")

    def test_fflush_stdout_present(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "fflush(stdout)" in _read(out_dir, "test_mod", "reg_host.cpp")
...
class TestCommHeaderRegType:
    ...
    def test_reg_type_unsigned_value(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "UNSIGNED" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_type_signed_value(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "SIGNED" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_type_float_value(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "FLOAT" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_reg_type_double_value(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "DOUBLE" in _read(out_dir, "test_mod", "reg_comm.hpp")
    ...
    def test_min_val_pointer_in_reg_info(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "min_val" in _read(out_dir, "test_mod", "reg_comm.hpp")

    def test_max_val_pointer_in_reg_info(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "max_val" in _read(out_dir, "test_mod", "reg_comm.hpp")
...
class TestRangeDefaultEmit:
    def test_default_array_emitted_for_register_with_default(self, tmp_path):
        ...
        _gen(rm, out)
        content = _read(out, "rng_mod", "reg_comm.cpp")
    ...
    def test_reg_cmp_le_emitted(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_cmp_le(" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_reg_cmp_le_signed_emitted(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_cmp_le_signed(" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_reg_cmp_le_float_emitted(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_cmp_le_float(" in _read(out_dir, "test_mod", "reg_comm.cpp")

    def test_reg_cmp_le_double_emitted(self, simple_rm, out_dir):
        _gen(simple_rm, out_dir)
        assert "reg_cmp_le_double(" in _read(out_dir, "test_mod", "reg_comm.cpp")
    ...
    def test_multiword_default_encodes_both_words(self, tmp_path):
        ...
        out = str(tmp_path / "out"); os.makedirs(out, exist_ok=True)
        _gen(rm, out)
        content = _read(out, "mwd_mod", "reg_comm.cpp")
    ...
    def test_signed_default_encodes_as_two_complement(self, tmp_path):
        ...
        out = str(tmp_path / "out"); os.makedirs(out, exist_ok=True)
        _gen(rm, out)
        content = _read(out, "sgn_mod", "reg_comm.cpp")
    ...
    def test_signed_min_encodes_as_two_complement(self, tmp_path):
        ...
        out = str(tmp_path / "out"); os.makedirs(out, exist_ok=True)
        _gen(rm, out)
        content = _read(out, "s8m_mod", "reg_comm.cpp")
