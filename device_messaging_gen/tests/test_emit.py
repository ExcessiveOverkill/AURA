...
def test_types_pragma_once():
    f = _generate()["drive_msg_types.hpp"]
    assert "#pragma once" in f


def test_types_message_count():
    f = _generate()["drive_msg_types.hpp"]
    assert "DRIVE_MESSAGE_COUNT = 6" in f


def test_types_message_id_enum():
    f = _generate()["drive_msg_types.hpp"]
    assert "enum class DriveMessageId" in f
...
def test_types_severity_enum():
    f = _generate()["drive_msg_types.hpp"]
    assert "enum class DriveMessageSeverity" in f
    assert "NONE = 0" in f
...
def test_types_command_enum():
    f = _generate()["drive_msg_types.hpp"]
    assert "enum class DriveMsgCommand" in f
...
def test_types_string_declarations():
    f = _generate()["drive_msg_types.hpp"]
    assert "drive_msg_get_name(" in f
    assert "drive_msg_get_desc(" in f
    assert "drive_msg_get_severity(" in f
    assert "drive_severity_label(" in f
    # Old full-path group function must be gone
    assert "drive_msg_get_group(" not in f
...
def test_types_accessor_structs():
    f = _generate()["drive_msg_types.hpp"]
    # Struct type names
    assert "struct _DriveMsg_System_Power" in f
    assert "struct _DriveMsg_System" in f
    assert "struct _DriveMsg_Communication" in f
    assert "struct _DriveMsg " in f or "struct _DriveMsg{" in f or "_DriveMsg {" in f
    # Leaf ID members
    assert "static constexpr DriveMessageId clock_fault" in f
    assert "static constexpr DriveMessageId low_voltage" in f
    assert "static constexpr DriveMessageId watchdog" in f
    # Nested group members
    assert "static constexpr _DriveMsg_System_Power power{};" in f
    assert "static constexpr _DriveMsg_System system{};" in f
    # Global accessor instance
    assert "drive_msgs" in f
...
def test_strings_message_values():
    f = _generate()["drive_msg_strings.cpp"]
    assert "drive_message_values" in f
...
def test_strings_delays():
    f = _generate()["drive_msg_strings.cpp"]
    assert "drive_message_delays" in f
    assert "5000u" in f
...
def test_strings_group_table():
    ...
    assert 'drive_msg_group_nodes' in f
    assert '"system"' in f
    assert '"power"' in f
...
def test_group_accessors_declared():
    f = _generate()["drive_msg_types.hpp"]
    assert "drive_msg_get_group_idx(" in f
    assert "drive_group_name(" in f
    assert "drive_group_parent(" in f
...
def test_header_class_declaration():
    f = _generate()["drive_msg.hpp"]
    assert "struct DriveMessaging" in f
...
def test_header_logging_methods():
    f = _generate()["drive_msg.hpp"]
    assert "DriveMessageSeverity add(DriveMessageId" in f
...
def test_header_clear_methods():
    f = _generate()["drive_msg.hpp"]
    assert "void clear_time(DriveMessageId" in f
    assert "void clear_payload(DriveMessageId" in f
    assert "void clear_hit_count(DriveMessageId" in f
    assert "void clear(DriveMessageId" in f
...
def test_header_query_methods():
    ...
    assert "bool is_active(DriveMessageId" in f
...
def test_impl_init():
    f = _generate()["drive_msg.cpp"]
    assert "DriveMessaging::init()" in f
    assert "memset" in f


def test_impl_add():
    f = _generate()["drive_msg.cpp"]
    assert "DriveMessaging::add(" in f
    assert "MSG_GET_TIME_US()" in f
    assert "_hit_counts" in f
    assert "_payloads" in f
...
def test_impl_clear():
    ...
    # Public clears exist
    assert "DriveMessaging::clear_time(" in f
    assert "DriveMessaging::clear_payload(" in f
    assert "DriveMessaging::clear_hit_count(" in f
    assert "DriveMessaging::clear(" in f
    assert "DriveMessaging::clear_all_times()" in f
    assert "DriveMessaging::clear_all_payloads()" in f
    assert "DriveMessaging::clear_all_hit_counts()" in f
    assert "DriveMessaging::clear_all()" in f
    ...
    assert "DriveMessaging::recalc_severity()" in f


def test_impl_is_active():
    f = _generate()["drive_msg.cpp"]
    assert "DriveMessaging::is_active(" in f
...
def test_impl_no_auto_recalc_in_clear():
    ...
    clear_fn = f[f.index("DriveMessaging::clear("):]
    clear_body = clear_fn[:clear_fn.index("DriveMessaging::clear_a")]
    assert "_recalc_severity" not in clear_body


def test_impl_comm_update():
    f = _generate()["drive_msg.cpp"]
    assert "DriveMessaging::comm_update()" in f
...
def test_custom_module_name():
    ...
    files = _generate(gen)
    assert "motor_ctrl_msg_types.hpp" in files
    assert "motor_ctrl_msg_strings.cpp" in files
    assert "motor_ctrl_msg.hpp" in files
    assert "motor_ctrl_msg.cpp" in files
    types = files["motor_ctrl_msg_types.hpp"]
    assert "MotorCtrlMessageId" in types
    assert "motor_ctrl_msgs" in types
    assert "MOTOR_CTRL_MESSAGE_COUNT" in types
...
def test_instanced_enum_members():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
...
def test_instanced_message_count():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
    assert "DRIVE_MESSAGE_COUNT = 7" in f


def test_instanced_unique_count_constant():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
    assert "DRIVE_UNIQUE_MSG_COUNT = 3" in f
...
def test_instanced_string_id_array():
    f = _generate(_make_instanced_gen())["drive_msg_strings.cpp"]
    assert "_drive_msg_string_id" in f
...
def test_instanced_accessor_array_in_types():
    ...
    assert "_DriveMsg_Motor motor[3]" in f


def test_instanced_struct_type_emitted():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
    assert "struct _DriveMsg_Motor" in f


def test_instanced_struct_non_static_members():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
    # Members inside _DriveMsg_Motor must NOT be static constexpr
    # Find the struct body
    start = f.index("struct _DriveMsg_Motor {")
    end = f.index("};", start)
    body = f[start:end]
...
def test_instanced_array_initializer_values():
    f = _generate(_make_instanced_gen())["drive_msg_types.hpp"]
    assert "DriveMessageId(0)" in f
    assert "DriveMessageId(2)" in f
    assert "DriveMessageId(4)" in f


def test_nested_instanced_enum_members():
    f = _generate(_make_nested_instanced_gen())["drive_msg_types.hpp"]
...
def test_nested_instanced_sub_struct():
    f = _generate(_make_nested_instanced_gen())["drive_msg_types.hpp"]
    assert "struct _DriveMsg_Motor_System" in f
    assert "struct _DriveMsg_Motor" in f


def test_nested_instanced_group_nodes_canonical():
    f = _generate(_make_nested_instanced_gen())["drive_msg_strings.cpp"]
